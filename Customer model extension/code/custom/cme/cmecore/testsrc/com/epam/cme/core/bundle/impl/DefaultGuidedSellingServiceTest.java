...
public class DefaultGuidedSellingServiceTest {
    ...
    public void testGetSubsequentExtraComponent() {
        ...
        given(Integer.valueOf(bundleTemplateService.getPositionInParent(deviceTemplateModel))).willReturn(
                Integer.valueOf(0));
        ...
        given(Integer.valueOf(bundleTemplateService.getPositionInParent(planTemplateModel))).willReturn(
                Integer.valueOf(1));
        ...
        BundleTemplateModel component = guidedSellingService.getRelativeSelectionComponent(masterCart,
                planTemplateModel, 1, 1);
        ...
        given(Integer.valueOf(bundleTemplateService.getPositionInParent(addOnTemplateModel))).willReturn(
                Integer.valueOf(2));

        given(addOnTemplateModel.getProducts()).willReturn(products);
        childTemplates.add(addOnTemplateModel);
        given(Boolean.valueOf(bundleComponentEditableChecker.isAutoPickComponent(addOnTemplateModel))).willReturn(
                Boolean.FALSE);
        given(Boolean.valueOf(bundleComponentEditableChecker.canEdit(masterCart, addOnTemplateModel, 1))).willReturn(
                Boolean.TRUE);
        given(bundleTemplateService.getRelativeBundleTemplate(planTemplateModel, 1)).willReturn(addOnTemplateModel);
        given(
                Boolean.valueOf(bundleTemplateService.containsComponenentProductsOfType(addOnTemplateModel,
                        DeviceModel.class, ServiceAddOnModel.class))).willReturn(Boolean.TRUE);
        ...
        // AddOn component comes after plan component and its dependencies are not met and it's the
        ...
        // Tv component comes after addon component and its dependencies are not met and its an
        // auto-pick and it does not have add-on proudcts
        ...
        given(Integer.valueOf(bundleTemplateService.getPositionInParent(tvTemplateModel))).willReturn(
                Integer.valueOf(3));

        given(Boolean.valueOf(bundleComponentEditableChecker.isAutoPickComponent(tvTemplateModel))).willReturn(
                Boolean.TRUE);
        given(Boolean.valueOf(bundleComponentEditableChecker.canEdit(masterCart, tvTemplateModel, 1))).willReturn(
                Boolean.FALSE);
        given(
                Boolean.valueOf(bundleTemplateService.containsComponenentProductsOfType(tvTemplateModel,
                        DeviceModel.class, ServiceAddOnModel.class))).willReturn(Boolean.FALSE);
        component = guidedSellingService.getRelativeSelectionComponent(masterCart, planTemplateModel, 1, 1);
        assertNull("Plan_Component should not have a subsequent component", component);

        // Tv component comes after addon component and its dependencies are met and its an
        // auto-pick and it does not have add-on proudcts
        given(Boolean.valueOf(bundleComponentEditableChecker.isAutoPickComponent(tvTemplateModel))).willReturn(
                Boolean.TRUE);
        given(Boolean.valueOf(bundleComponentEditableChecker.canEdit(masterCart, tvTemplateModel, 1))).willReturn(
        ...
        // Tv component comes after addon component and its dependencies are met and its not an
        // auto-pick and it does not have add-on proudcts
        given(Boolean.valueOf(bundleComponentEditableChecker.isAutoPickComponent(tvTemplateModel))).willReturn(
                Boolean.FALSE);
        given(Boolean.valueOf(bundleComponentEditableChecker.canEdit(masterCart, tvTemplateModel, 1))).willReturn(
        ...
        // Tv component comes after addon component and its dependencies are met and its not an
        // auto-pick and it has add-on proudcts
    ...
    public void testIsComponentToBeDisplayedOnExtrasPageWhenNoTemplate() {
        ...
        guidedSellingService.isComponentToBeDisplayedOnGuidedSellingSelectPage(new CartModel(), null, 1,
                ServiceAddOnModel.class);
    }
}
