...
public class AnalyticsPropertiesBeforeViewHandler implements BeforeViewHandler {
    ...
    public void beforeView(final HttpServletRequest request, final HttpServletResponse response,
        ...
        addHostProperty(serverName, modelAndView, ThirdPartyConstants.Google.ANALYTICS_TRACKING_ID,
        ...
        // Lookup a currency specific jirafe site id first, and only if it is missing fallback to
        // the default site id
        final String currencyIso = commonI18NService.getCurrentCurrency().getIsocode().toLowerCase();
        final String currencySpecificJirafeSiteId = hostConfigService.getProperty(ThirdPartyConstants.Jirafe.SITE_ID
                + "." + currencyIso, serverName);
