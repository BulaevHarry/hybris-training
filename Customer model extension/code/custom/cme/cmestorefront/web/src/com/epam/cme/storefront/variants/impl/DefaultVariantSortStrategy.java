 ...
 * Use {@link #sortingFieldsOrder} property to set up attributes to sort by. E.g give A,B attributes
 * and sorting will be performed first against A attribute and if two values are considered equal
 * (by A) then values of B attribute are compared.<br>
    ...
    public int compare(final VariantOptionData product1, final VariantOptionData product2) {
        ...
        if (fields != null) {
            for (final String field : fields) {
                final int result = getComparator(field).compare(getVariantValue(field, product1),
                        getVariantValue(field, product2));
                if (result != 0) {
                    return result;
                }
            }
