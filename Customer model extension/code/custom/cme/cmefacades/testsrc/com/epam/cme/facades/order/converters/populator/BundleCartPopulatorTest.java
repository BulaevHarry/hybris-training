...
public class BundleCartPopulatorTest {
    ...
    public void testaddEmptyBundleComponents2ToEntries() {
        ...
        for (final BundleTemplateModel child : rootTemplate.getChildTemplates()) {
            when(Boolean.valueOf(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 1))).thenReturn(
                    Boolean.TRUE);
        }
    ...
    public void testGetLastBundlesEmptyChildrenGetAdded() {
        ...
        for (final BundleTemplateModel child : rootTemplate2.getChildTemplates()) {
            when(Boolean.valueOf(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 2))).thenReturn(
                    Boolean.TRUE);
        }
        orderEntries.add(createOrderEntry(1, "Type1child1", root1));
    ...
    public void testNonEditableShouldNotGetAdded() {
        ...
        for (int i = 1; i < rootTemplate.getChildTemplates().size(); i++) {
            when(
                    Boolean.valueOf(bundleCommerceCartService.checkIsComponentEditable(masterCart, rootTemplate
                            .getChildTemplates().get(i), 1))).thenReturn(Boolean.TRUE);
        }
        when(
                Boolean.valueOf(bundleCommerceCartService.checkIsComponentEditable(masterCart, rootTemplate
                        .getChildTemplates().get(0), 1))).thenReturn(Boolean.FALSE);
    ...
    private OrderEntryData createOrderEntry(final int bundleNo, final String bundleId,
            final BundleTemplateData rootBundleTemplate) {
