...
/**
 * Modify the cart converter to show all bundle components of a package in a cart. This means that
 * even those components are displayed for which there is no product in the cart yet. Also show the
 * first invalid bundle component in the cart (if it exists).
 */
public class BundleCartPopulator<S extends CartModel, T extends CartData> extends AbstractBundleOrderPopulator<S, T> {
    ...
    /**
     * Modify populate method to set the first incomplete bundle component in the cart
     * {@link CartModel}
     */
    @Override
    ...
    /**
     * Modify addEntries method to add bundle components that belong to a bundle template but not
     * added yet to cart
     */
    @Override
    ...
    /**
     * Method to merge existing cart entries {@link OrderEntryData} and empty cart entries
     * {@link OrderEntryData}. If cart entry is 1) a standalone product return orderentries because
     * there is nothing to add. 2) a bundle product, get all the bundle components of the root
     * bundle. Merge the retrieved bundle components with bundle components already added to the
     * cart - while maintaining the relative order of bundle components
     * 
     * @param entries
    ...
    protected Collection<OrderEntryData> mergeEntries(final Collection<OrderEntryData> entries, final S cart) {
        ...
        final BundleTemplateModel rootBundle = getBundleTemplateService().getBundleTemplateForCode(
                rootTemplateData.getId(), rootTemplateData.getVersion());
    ...
    /**
     * Method to add an empty {@link OrderEntryData} to the bundle cart view. The quantity is
     * currently being set to some invalid value like -1. And BillingTimeData is also being set to a
     * default value.
     * 
     * @param bundleNo
    ...
    protected OrderEntryData createEmptyOrderEntryData(final int bundleNo, final BundleTemplateModel rootBundle,
        ...
        emptyOrderEntry.setValid(getBundleCommerceCartService().checkIsComponentSelectionCriteriaMet(masterCart,
    ...
    protected void addCartInvalidMessage(final S source, final T target) {
        final BundleTemplateModel bundleTemplate = getBundleCommerceCartService()
                .getFirstInvalidComponentInCart(source);
        if (bundleTemplate != null) {
            final String cartInvalidMessage = getL10NService().getLocalizedString(
                    "bundleservices.validation.cartcomponentisinvalid",
                    new Object[] { bundleTemplate.getParentTemplate().getName(), bundleTemplate.getName() });
