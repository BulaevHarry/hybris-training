...
public class DashboardPopulator<SOURCE extends DashboardPopulatorParameters, TARGET extends DashboardData> implements
    ...
    /**
     * The dashbord boxes that have been created in a generic way are refactored to telco needs.
     * Please see inline comments for more information on the changes.
     * 
     * @param dashboardBoxes
     *            the generic dashboard boxes, one box for each existing component
     * @return the telco specific dashboard boxes, always 3 boxes, one for the device, one for the
     *         service plan and one for all service addons.
     */
    protected List<DashboardBoxData> mergeTelcoSpecificBoxes(final List<DashboardBoxData> dashboardBoxes) {
        ...
        // if there is no device box - e.g. because of a sim only package - then we are adding one
