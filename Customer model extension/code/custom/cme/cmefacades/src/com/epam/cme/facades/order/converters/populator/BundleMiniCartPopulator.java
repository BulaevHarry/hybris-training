...
/**
 * Slightly changed behavior to display the last added item(s) instead of just the first x items in
 * the cart, which does not make any sense in terms of bundles.
 */
public class BundleMiniCartPopulator<S extends CartModel, T extends CartData> extends
    ...
    /**
     * Add only those cart entries that will be displayed in the mini cart (= last modified
     * records). Existing entries in the target are not replaced but filtered/sorted so that only
     * the last modified records stay
     * 
     */
    @Override
